...
class MainWindow(QMainWindow):
    ...
    def createActions(self):
        ...
        self.dnn_classify_act2 = QAction("Deep Neural Network hokpip")
        self.dnn_classify_act2.setStatusTip("Classify clothing image type using Deep Neural Network")
        self.dnn_classify_act2.triggered.connect(self.classifyClothingWithDnnHokpip) 
        
        self.dnn_classify_act3 = QAction("Deep Neural Network with 90 ac rotate")
        self.dnn_classify_act3.setStatusTip("Classify clothing image type using Deep Neural Network")
        self.dnn_classify_act3.triggered.connect(self.classifyClothingWithDnnrt) 
    ...
    def createMenu(self):
        ...
        classify_menu.addAction(self.dnn_classify_act2)
        classify_menu.addAction(self.dnn_classify_act3)
    ...
    def createToolsDockWidget(self):
        ...
        self.classify_dnn2 = QPushButton("DNN Clothing Classifier hokepip")
        self.classify_dnn2.setMinimumSize(QSize(130, 40))
        self.classify_dnn2.setStatusTip("Classify clothing image type using Deep Neural Network")
        self.classify_dnn2.clicked.connect(self.classifyClothingWithDnnHokpip)
        
        self.classify_dnn3 = QPushButton("DNN Clothing Classifier with 90 ac rotate")
        self.classify_dnn3.setMinimumSize(QSize(130, 40))
        self.classify_dnn3.setStatusTip("Classify clothing image type using Deep Neural Network")
        self.classify_dnn3.clicked.connect(self.classifyClothingWithDnnrt)
        ...
        dock_v_box.addStretch(1)
        dock_v_box.addWidget(self.classify_dnn2)
        dock_v_box.addStretch(1)
        dock_v_box.addWidget(self.classify_dnn3)
    ...
    def classifyClothingWithDnnHokpip(self):
        """Classify clothing images using deep neural net model."""
        input_width = 28
        input_height = 28
        temp_file = 'temp.jpeg'
        ...
        if B is not None: 
            clothing_class = self.dnn_predicthokpip(B)
        else:
           QMessageBox.about(self, "DNN Cloth Classifier", "Sorry! input image is empty.")
    
    def dnn_predicthokpip(self, image_arr):
        class_names = np.array(["T-shirt/top", "Trouser", "Pullover", "Dress", "Coat",
               "Sandal", "Shirt", "Sneaker", "Bag", "Ankle boot"])

        model = keras.models.load_model("my_fashion_mnist_model_hokepip")
        y_proba = model.predict(image_arr)
        print(y_proba)
        y_pred = class_names[y_proba.argmax(axis=-1)][0]
    ...
    def classifyClothingWithDnnrt(self):
        """Classify clothing images using deep neural net model."""
        input_width = 28
        input_height = 28
        temp_file = 'temp.jpeg'
        ...
        if B is not None: 
            clothing_class = self.dnn_predictrt(B)
        else:
           QMessageBox.about(self, "DNN Cloth Classifier", "Sorry! input image is empty.")
    
    def dnn_predictrt(self, image_arr):
        class_names = np.array(["T-shirt/top", "Trouser", "Pullover", "Dress", "Coat",
               "Sandal", "Shirt", "Sneaker", "Bag", "Ankle boot"])

        model = keras.models.load_model("fashion_model_hokepip_with_rotate")
        y_proba = model.predict(image_arr)
        print(y_proba)
        y_pred = class_names[y_proba.argmax(axis=-1)][0]
